...
public class ServiceFeed {
    ...
    public ArrayList<Feed> getFeed(int idEvent) {
        String url =   "http://localhost/MonProjet1/web/app_dev.php/api/feed/"+idEvent;
        req.setUrl(url);
        req.setPost(false);
    ...
    public boolean ajouter(int idUser,int idEvent,String desc) {
        String url =  "http://localhost/MonProjet1/web/app_dev.php/api/addFeed/"+idUser+"/"+idEvent+"/"+desc;
        req.setUrl(url);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
